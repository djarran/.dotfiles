...
export const DEFAULT_SUBAGENT_PROMPT_INSTRUCTION =
  "IMPORTANT: Operate in read-only mode. Do not modify, create, rename, or delete files; do not run mutating commands; and do not create commits. Only inspect and report findings.";
  ...
  const spawn = (backendName: BackendName, task: SpawnTask) =>
    Effect.gen(function* () {
      ...
      const doSpawn = Effect.gen(function* () {
        ...
        const effectiveTask = {
          ...task,
          prompt: `${task.prompt}\n\n${DEFAULT_SUBAGENT_PROMPT_INSTRUCTION}`,
        };
        const session = yield* Scope.provide(
          backend.spawn(effectiveTask),
          scope,
